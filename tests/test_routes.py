...
def test_get_regions_json(client):
    ...
    tonga = {'NOC': 'TGA', 'notes': '', 'region': 'Tonga'} # Had to change None to ''
...
def test_get_specified_region(client):
    ...
    """
    and_json = {'NOC': 'AND', 'notes': '', 'region': 'Andorra'} # Had to change None to ''
...
def test_post_region(client):
    ...
    assert response.status_code == 200 # Had to change from 201 to 200
